...
class VacancyBoard:
    ...
    def InitializeTalentReqs(self):
        ...
        for g in num_vacs.keys():
            ...
            '''
            n_auth = round(self.recruit_rate[grd] * num_vacs[grd])
            for n in range(n_recruit):
            
                loc=l_locs.pop()
                newage = self.model.agemodel.GetRandAgeValue(grd,self.model.noise)
                v = Vacancy("NEWHIRE", loc, edlvl.pop(), grd, salvl.pop(), loslvl.pop(),newage)
                if not loc in self.loc_vacs: self.loc_vacs[loc]=0
                self.loc_vacs[loc] += 1
                #self._newhires[int(grd)].append((loc,v))
            '''
    ######################################################################            
    def CalcVacancyRates(self):
        ...
        '''OLD
        for grd in self.vacancy_rate.keys():
            if self._authorized_gs[grd] > 0:
                self.vacancy_rate[grd] = self.vacancies_gs[grd] / self.model.authorized_gs[grd]
            else:
                self.vacancy_rate[grd] = 0.0
